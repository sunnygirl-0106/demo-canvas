...
describe('模式能力与有效素材', () => {
  ...
  it('两个参考 Tab 互斥：只收图的型号进参考图，收多模态的进全能参考', () => {
    ...
    // 整类不参与说的是同一句话，和首尾帧不用视频一样，不逐个点名
    expect(wan.find((t) => t.k === 'refImage')!.note).toBe('视频不参与本次生成')
  ...
  it('超出配额与用不上的素材都给得出理由，且不进入有效输入', () => {
    ...
    expect(skipped.every((x) => x.reason === '视频不参与本次生成')).toBe(true)
  ...
  it('源视频时长不合规不是事后的黄字，是 Tab 进不去', () => {
    ...
    expect(states.find((t) => t.k === 'edit')!.reason).toBe('视频 IJKL 的时长需在 4–30 秒之间；可切换至 Seedance 2.0')
    ...
    expect(tabStates(conn, tiny, 'sd2.5').find((t) => t.k === 'edit')!.reason).toBe('视频 IJKL 的时长需在 2–30 秒之间')
  ...
  it('用不上的素材不在面板上占位，改由 Tab 悬浮说明', () => {
    ...
    expect(tabStates(['v', 'w'], short, 'sd2.5', () => 'v').find((t) => t.k === 'edit')!.note)
      .toBe('视频 MNOP 的时长需在 4–30 秒之间，不参与本次生成')
    ...
    expect(tabStates(conn, get, 'sd2.5').find((t) => t.k === 'frames')!.note).toBe('视频不参与本次生成')
    expect(tabStates(['a', 'b'], get, 'sd2.5').find((t) => t.k === 'frames')!.note).toBe('')
    // 超过配额同样只在 Tab 上说：Wan 图生视频只收 1 张图
    expect(tabStates(['a', 'b'], get, 'wan2.2-i2v-a14b').find((t) => t.k === 'refImage')!.note).toContain('最多支持 1 张图片')
