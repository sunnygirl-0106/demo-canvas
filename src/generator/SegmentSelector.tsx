...
interface Props { mat: Mat; model: Model; range: TimeRange | null; onChange: (range: TimeRange | null) => void }
export default function SegmentSelector({ mat, model, range, onChange }: Props) {
  ...
  const duration = mat.dur ?? 0; const disabled = !!(sourceError(mat.dur, mat.ready, 'edit', model) || error)
  ...
  return <section className="segment-selector nodrag nowheel" aria-label="片段选择器" id="segment-selector">
    ...
    <div className="timeline-heading"><span>{range ? `${timecode(range.start)}–${timecode(range.end)} · 已选 ${range.end - range.start} 秒` : '在轨道上拖出要作用的片段'} </span><span>原片 {mat.dur != null ? fmt(mat.dur) : '读取中'}</span></div>
